...
#from diffusers import ControlNetModel as ControlNetModel_diffuser

import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

#from unipc import UniPCMultistepScheduler as customize_UniPC
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--grad_scale_KL", type=float, default=0, help="Scale divided for grad loss value."
    )

    parser.add_argument(
        "--beta_dpo", type=float, default=2500, help="Beta scaler for DPO. this controls step size."
    )
...
def make_train_dataset(args, tokenizer, accelerator, split='train', codec=None):
    ...
    def preprocess_train(examples):
        ...
        winner_col = conditioning_image_column

        loser_col = winner_col.replace('winner','loser')

        pil_winner = [image.convert("RGB") for image in examples[winner_col]]
        winners = [conditioning_image_transforms(image) for image in pil_winner]

        pil_loser = [image.convert("RGB") for image in examples[loser_col]]
        losers = [conditioning_image_transforms(image) for image in pil_loser]
        ...
        examples["winners"] = winners
        examples["losers"] = losers
...
def collate_fn(examples):
    ...
    winners = torch.stack([example["winners"] for example in examples])
    winners = winners.to(memory_format=torch.contiguous_format).float()

    losers = torch.stack([example["losers"] for example in examples])
    losers = losers.to(memory_format=torch.contiguous_format).float()
...
"""
def get_loss(input_images,controlnet_image,encoder_hidden_states,vae,args,unet,controlnet,noise_scheduler,weight_dtype,timesteps,num_chunks):
    latents = vae.encode(input_images.to(dtype=weight_dtype)).latent_dist.sample()
    latents = latents * vae.config.scaling_factor
    chunks = latents.chunk(num_chunks) ## chunk into different chunks
    noise = torch.randn_like(chunks[0]) ## share same noise
    noise = noise.repeat(num_chunks,1,1,1) ## repeat the same noise for them
    timesteps = timesteps.repeat(num_chunks)
    #controlnet_image = controlnet_image.repeat(num_chunks,1,1,1)
    encoder_hidden_states = encoder_hidden_states.repeat(num_chunks,1,1)
...
def compute_DPO_loss(model_losses_w,model_losses_l,ref_losses_w,ref_losses_l,args):
    model_diff = model_losses_w - model_losses_l # These are both LBS (as is t)
    ref_diff = ref_losses_w - ref_losses_l # These are both LBS (as is t)
    scale_term = -0.5 * args.beta_dpo
    scale_term = 1/((scale_term * (model_diff.detach() - ref_diff)).detach().exp() + 1)
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        ref_controlnet = ControlNetModel.from_unet(unet)
    ...
    if args.use_ema:
        ...
        ema_controlnet = EMAModel(ema_controlnet.parameters(), model_cls=ControlNetModel, model_config=controlnet.config)
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            ref_controlnet.enable_xformers_memory_efficient_attention()
        else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        DPO_loss_per_epoch = 0.

        train_loss, train_pretrain_loss, train_reward_loss, train_DPO_loss = 0., 0., 0., 0.

        for step, batch in enumerate(train_dataloader):
            # TODO: Clean the code)
            if global_step == 0:
                with torch.no_grad():
                    ema_controlnet.copy_to(ref_controlnet.parameters())
                    ref_controlnet.requires_grad_(False)
            ...
            with accelerator.accumulate(controlnet):
                ...
                bsz = latents.shape[0]
                controlnet_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype).clone()
                # Concatenate winners, losers, and pixel values along the batch dimension
                timesteps = torch.randint(args.timestep_sampling_start, args.timestep_sampling_end, (bsz,), device=accelerator.device)
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
                pretrain_loss = F.mse_loss(pred.float(), target.float(), reduction="mean")
                ...
                model_controlnet_image = torch.cat([batch["winners"].to(dtype=weight_dtype).clone(),batch["losers"].to(dtype=weight_dtype).clone()])  # image condition
                ...
                model_timesteps = torch.randint(args.timestep_sampling_start, args.timestep_sampling_end, (bsz,), device=accelerator.device)
                ...
                with torch.no_grad(): # Get the reference policy (unet) prediction
                    ref_input_images = batch["pixel_values"].repeat(2,1,1,1)
                    ref_controlnet_image = torch.cat([batch["winners"].to(dtype=weight_dtype).clone(),batch["losers"].to(dtype=weight_dtype).clone()])  # image condition
                ...
                implicit_acc = 1 - (inside_term > 0).sum().float() / inside_term.size(0)
                ...
                reverse_scale_term = 1/(((-0.5 * args.beta_dpo) * (- model_diff + ref_diff)).detach().exp() + 1)
                reverse_DPO_loss = torch.clamp(model_losses_l - model_losses_w + args.margin,min=0.)
                #DPO_loss =  model_losses_w - model_losses_l
                inside_DPO_loss = DPO_loss.clone().detach().mean()

                

                DPO_loss = 0.9*scale_term * DPO_loss + 0.1 * reverse_scale_term*reverse_DPO_loss
                #DPO_loss =  torch.clamp(model_losses_w - model_losses_l + args.margin,min=0.)
                if dpo_mask.sum() == 0:
                ...
                #DPO_loss = (DPO_loss*time_step_weight).mean()
                ...
                reward_loss = torch.zeros_like(pretrain_loss)
                #DPO_loss = DPO_loss.detach()

                if global_step < args.start_step:
                    loss = pretrain_loss  + 0. * DPO_loss + reward_loss * 0
                else:
                    loss = (args.grad_scale * pretrain_loss  +  DPO_loss + reward_loss * 0).detach()
                ...
                avg_DPO_loss = accelerator.gather(inside_DPO_loss.repeat(args.train_batch_size)).mean()
            ...
            if accelerator.sync_gradients:
                ...
                DPO_loss_per_epoch += train_DPO_loss
            ...
            logs = {
                ...
                "DPO_loss_step": inside_DPO_loss.detach().item(),
        ...
        logs = {
            ...
            "DPO_loss_epoch": DPO_loss_per_epoch / len(train_dataloader),
