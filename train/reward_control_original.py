...
def main(args):
    ...
    noise_scheduler = noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler")
    ...
    if args.task_name in ['canny', 'segmentation']:
        controlnet.canny_or_seg = True
    else:
        controlnet.canny_or_seg = False
    if args.use_ema:
        ema_controlnet = ControlNetModel.from_ControlNet(args.controlnet_model_name_or_path)
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {"name": args.output_dir.split('/')[-1]},"settings": wandb_settings})
            ...
            with accelerator.accumulate(controlnet):
                ...
                if args.conditioning_image_column == "canny":
                    ...
                    with torch.no_grad():
                        ...
                        tensor = controlnet_image[0]               # just to be safe
                        arr = (tensor * 255).byte().cpu().numpy() # now uint8, shape [C, H, W]
                        arr = np.transpose(arr, (1, 2, 0))        # → [H, W, C]

                        # if C==1 you can squeeze it and use mode="L"
                    ...
                    with torch.no_grad():
                        ...
                        labels = controlnet_image.detach().clone()
