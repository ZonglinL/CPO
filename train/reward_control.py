    ...
    if args.conditioning_image_column == "canny":
        ...
        with autocast():
            with torch.no_grad():
                ...
                depth_input = torchvision.transforms.functional.resize(validation_conditions, size=(new_height, new_width), interpolation=transforms.InterpolationMode.BCUBIC)
...
def main(args):
    ...
    if 'ADE' in args.dataset_name:
        reward_model = UperNetForSemanticSegmentation.from_pretrained("openmmlab/upernet-convnext-xlarge")
    elif args.task_name == 'depth':
        reward_model = DPTForDepthEstimation.from_pretrained("Intel/dpt-hybrid-midas")
    else:
        reward_model = get_reward_model(args.task_name, args.reward_model_name_or_path)
    ...
    if accelerator.is_main_process:
        ...
        wandb_settings = Settings(init_timeout=500)
            ...
            with accelerator.accumulate(controlnet):
                ...
                if args.conditioning_image_column == "canny":
                    ...
                    with torch.no_grad():
                        ...
                        labels = outputs.detach().clone() ## 518 for labels
                        ...
                        outputs = jpeg_smooth_tensor(outputs) ## smooth for condition
                ...
                if args.task_name == 'depth':
                    ...
                    pass
                #! Warning: Not sure if pose model should use ImageNet normalziation
                else:
                    image = normalize(image, (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))

                # reward model inference
                if args.task_name == 'pose':
                    feats = reward_model.extract_feat(image.to(accelerator.device))
                    outputs = reward_model.head(feats)
                ...
                elif args.task_name in ['lineart', 'hed','depth']:
                    pass
                else:
                    labels = batch["labels"]
