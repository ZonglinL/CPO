# Path to the controlnet weight (can be huggingface or local path)
export CONTROLNET_DIR="ucfzl/FLUX_Lineart_CPO"

# How many GPUs and processes you want to use for evaluation.
export NUM_GPUS=2
# Guidance scale and inference steps
export SCALE=3.5
export NUM_STEPS=20

# Generate images for evaluation
# If the command is interrupted unexpectedly, just run the code again. We will skip the already generated images.
accelerate launch --config_file "eval/config4.yml" --main_process_port=10028  eval/eval_flux.py --task_name='lineart' --dataset_name='limingcv/MultiGen-20M_canny_eval' --dataset_split='validation' --condition_column='image' --prompt_column='text' --model_path $CONTROLNET_DIR --guidance_scale=${SCALE} --num_inference_steps=${NUM_STEPS} --resolution=1024


# Path to the above generated images
# guidance_scale=7.5, sampling_steps=20 by default
export DATA_DIR="work_dirs/eval_dirs/MultiGen-20M_canny_eval/validation/${CONTROLNET_DIR//\//_}_${SCALE}-${NUM_STEPS}"

# Run the evaluation code
python3 eval/eval_edge.py --task lineart --root_dir ${DATA_DIR} --num_processes ${NUM_GPUS}

find "${DATA_DIR}/images" -type f -name '*lineart*' -delete

python eval/eval_fid.py --task_name='lineart' --dataset_name='limingcv/MultiGen-20M_canny_eval' --dataset_split='validation' --condition_column='image' --prompt_column='text'  --model_path $CONTROLNET_DIR --guidance_scale=${SCALE} --num_inference_steps=${NUM_STEPS}

export IMAGE_DIR="${DATA_DIR}/images/"

python eval/eval_clip.py --generated_image_dir=$IMAGE_DIR --dataset='limingcv/MultiGen-20M_canny_eval'