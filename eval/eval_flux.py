...
def main(args):
    ...
    with distributed_state.main_process_first():
        if args.task_name == 'depth':
            ...
            model = AutoModelForDepthEstimation.from_pretrained("depth-anything/Depth-Anything-V2-Large-hf").to(distributed_state.device)
            model.eval()
    ...
    with distributed_state.split_between_processes(list(range(len(dataset)))) as local_idxs:
        ...
        for idx in local_idxs:
            ...
            if args.task_name == 'canny':
                ...
                low_threshold = 0.15 # random.uniform(0, 1)
                high_threshold = 0.3 # random.uniform(low_threshold, 1)
            ...
            if args.task_name == 'canny_cv2':
                ...
                with torch.no_grad():
                    ...
                    condition = outputs.predicted_depth.cpu() ## 1 h w
                    condition = condition - condition.min()
                    condition = condition / condition.max()
                    #condition = jpeg_smooth_tensor(condition)
                
                condition = F.to_pil_image(condition, 'L').convert('RGB')
                condition = condition.resize((args.resolution, args.resolution), Image.Resampling.BICUBIC)
                label = condition
            elif args.task_name == 'seg':
                controlnet.canny_or_seg = True            
            ...
            if args.task_name == 'canny':
                ...
                with torch.no_grad():
                    ...
                    depth_maps = [F.to_pil_image((x - x.min())/(x.max() - x.min())).convert('RGB').resize((args.resolution, args.resolution), Image.Resampling.BICUBIC) for x in predicted_depth]
